...
abstract class GxActiveRecord extends CActiveRecord {
	...
	public static function saveMultiple($models, $runValidation = true, $options = array()) {
		...
		try {
			// Start the transaction if required.
			if ($options['withTransaction'] && (GxActiveRecord::model()->getDbConnection()->getCurrentTransaction() === null)) {
				$transacted = true;
				$transaction = GxActiveRecord::model()->getDbConnection()->beginTransaction();
			} else
				$transacted = false;
			...
			foreach ($models as $modelItem) {
				...
				// Save the model
				if (!GxActiveRecord::model()->save($modelOptions['runValidation'], $modelOptions['attributes'])) {
					if ($transacted)
						$transaction->rollback();
					return false;
				}
